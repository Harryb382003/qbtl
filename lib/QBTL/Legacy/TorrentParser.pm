...
sub locate_items {
    Logger::info("Torrent locator complete, passing to TorrentParser");
    my ($args) = @_;
    $args ||= {};
...
sub _process_bucket {
    ...
    while (@files) {
        ...
        foreach my $file_path (@batch) {
            Logger::info(__LINE__ . " [BUCKET:$bucket] Would load into API: $file_path");
            # TODO: replace this stub with actual add_torrent call later
        }

        Logger::info(__LINE__ . "[ BUCKET:$bucket] Completed batch of " . scalar(@batch));
...
sub extract_metadata {
    ...
    foreach my $file_path (@files) {
        ...
        if ($@ || !defined $info_bencoded) {
            Logger::warn("[TorrentParser] Failed to bencode info dict in $file_path: $@");
            next;
        }
        my $infohash = sha1_hex($info_bencoded);

        # --- Name ---
        my $torrent_name = $info->{name} // '';
