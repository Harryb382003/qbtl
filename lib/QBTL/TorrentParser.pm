...
sub new {
  my ( $class, $args ) = @_;
  my $self =
      {all_torrents => $args->{all_torrents}, opts => $args->{opts},};
  bless $self, $class;
...
sub torrent_paths {
  ...
  elsif ( defined $opts->{export_dir_fin}
          && length $opts->{export_dir_fin} )
  {
    push @prune, $opts->{export_dir_fin};
  }
...
sub locate_items {
  ...
  QBTL::Logger::debug( "#\tlocate_items" );
  QBTL::Utils::start_timer( "locate_items" );
  ...
  QBTL::Logger::info(
            "[MAIN] Located " . scalar( @results ) . " item(s) via Spotlight" );
  QBTL::Utils::stop_timer( "locate_items" );

  QBTL::Logger::summary( "Torrent files located\t\t" . scalar( @results ) );
...
sub extract_metadata {
  QBTL::Logger::debug( "# --- EXTRACTING METADATA --- #" );
  my ( $self, $qbt ) = @_;
  my $opts  = $self->{opts};
  ...
  QBTL::Logger::info(
            "Primed \\%seen with " . scalar( keys %seen ) . " qbt infohashes" );

  foreach my $file_path ( @files ) {
    QBTL::Logger::trace( "[TorrentParser] Processing $file_path" );
    ...
    my $raw;
    {
      ...
      open my $fh, '<:raw', $file_path or do {
        QBTL::Logger::error( "[TorrentParser] Failed to open $file_path: $!" );
        next;
      };
      $raw = <$fh>;
      close $fh;
    ...
    if ( $@ || ref( $t ) ne 'HASH' ) {
      QBTL::Logger::warn( "[TorrentParser] Failed to bdecode $file_path: $@" );
      next;
    }

    my $info = $t->{info};
    if ( ref( $info ) ne 'HASH' ) {
      QBTL::Logger::warn( "[TorrentParser] Missing info dict in $file_path" );
      next;
    }
    ...
    if ( ref( $info->{files} ) eq 'ARRAY' ) {
      for my $f ( @{$info->{files}} ) {
        ...
        my $p =
            exists $f->{'path.utf-8'} ? $f->{'path.utf-8'} : $f->{path};
...
# Prefix root name for multi-file torrents so we can strip later for savepath derivation
    ...
    unless ( @files_norm ) {
      QBTL::Logger::warn(
                       "[TorrentParser] No usable file entries in $file_path" );
      next;
    }
  ...
  for my $bucket ( sort keys %bucket_uniques ) {
    ...
    QBTL::Logger::summary(
                         sprintf(
                                  "   %-20s  total=%6d  uniques=%6d  dupes=%6d",
                                  $bucket, $t, $u, $d ) );
  }
...
sub report_collision_groups {
  ...
  if ( exists $bucket_groups->{kitchen_sink} ) {
    for my $name ( sort keys %{$bucket_groups->{kitchen_sink}} ) {
      ...
      #                 QBTL::Logger::info("\n[COLLIDER] $name");
      #                 QBTL::Logger::info("\t$_->{source_path}") for @$entries;
      }
    }
  }

  QBTL::Logger::summary(
                    " Filename collision groups observed:\t$collision_groups" );
}
...
#   else
#   {
#     QBTL::Logger::info("[normalize_filename] Normalization disabled (mode=0)");
#   }
#
...
#     || $stats->{normalize_dry})
#   {
#     QBTL::Logger::summary("    Normalized (all):       $stats->{normalized}")
#         if $stats->{normalized};
#     QBTL::Logger::summary("    Normalized (colliders): $stats->{normalized_coll}")
#         if $stats->{normalized_coll};
#     QBTL::Logger::summary("    Dry-run count:          $stats->{normalize_dry}")
#         if $stats->{normalize_dry};
#   }
#   else
#   {
#     QBTL::Logger::summary("    No normalization changes performed");
#   }
#
#   return;
# }
